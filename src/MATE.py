...
class MATETableExtraction:
    ...
    def __init__(
        ...
        for q in query_column_list:
            self.input_data[q] = (
                self.input_data[q]
                .apply(lambda x: get_cleaned_text(x))
                .replace("", np.nan)
                .replace("nan", np.nan)
    ...
    def XASH(self, token: str, hash_size: int = 128) -> int:
        ...
        selected_chars = [
            y[0]
            for y in sorted(
                cnt_dict.items(), key=lambda x: (x[1], x[0]), reverse=False
            )[: self.number_of_ones]
        ]
        ...
        n = int(result)
        d = int(
            (length_bit_start * (len(token) % (hash_size - length_bit_start)))
        ...
        return int(result) | int(
            math.pow(2, len(token) % (hash_size - length_bit_start))
            * math.pow(2, length_bit_start)
        )
    ...
    def generate_SIM_hash(
        self, hash_dict: Dict, token: str, hash_size: int
    ) -> Tuple[Dict, int]:
    ...
    def generate_CITY_hash(
        hash_dict: Dict, token: str, hash_size: int
    ) -> Tuple[Dict, int]:
    ...
    def ICS(self) -> int:
        ...
        self.query_columns.insert(
            0, self.query_columns.pop(self.query_columns.index(best_query))
        )
    ...
    def run_SCI_system(self, run_ics: bool = False, active_pruning: bool = True):
        ...
        table_row = self.dbh.get_concatenated_posting_list(
            self.dataset_name,
        ...
        for tableid in tqdm(
            sorted(
                table_dictionary, key=lambda k: len(table_dictionary[k]), reverse=True
            )
        ):
            set_of_rowids = set()
            ...
            if (
                active_pruning
                and len(top_joinable_tables) >= self.top_k
                and top_joinable_tables[0][0] >= len(hitting_posting_list_concatenated)
            ):
                pruned = True
            if active_pruning and (
                (
                    self.is_min_join_ratio_absolute
                    and len(hitting_posting_list_concatenated) < self.min_join_ratio
                )
                or (
                    not self.is_min_join_ratio_absolute
                    and len(hitting_posting_list_concatenated)
                    < round(self.min_join_ratio * self.input_size)
                )
            ):
                pruned = True
            ...
            for hit in sorted(hitting_posting_list_concatenated):
                if (
                    ...
                    and (
                        (
                            len(hitting_posting_list_concatenated)
            ...
            if pruned or len(candidate_external_row_ids) >= row_block_size:
                ...
                for i in np.arange(len(candidate_table_rows)):
                    ...
                    match, matched_columns = self.evaluate_rows(
                        candidate_input_rows[i], col_dict
                    )
                    total_approved += 1
                    if match:
                        total_match += 1
    ...
    def run_system_multi_sci(self, run_ics: bool = False, active_pruning: bool = True):
        ...
        table_row_1 = self.dbh.get_concatenated_posting_list(
            self.dataset_name,
            self.query_columns[0],
            self.input_data[self.query_columns[0]],
        )
        ...
        table_row_2 = self.dbh.get_concatenated_posting_list(
            self.dataset_name,
            self.query_columns[1],
            self.input_data[self.query_columns[1]],
        )
        ...
        for tableid in sorted(
            table_dictionary_1, key=lambda k: len(table_dictionary_1[k]), reverse=True
        )[:max_table_check]:
            for i in table_dictionary_1[tableid]:
                ...
                if (
                    tableid in table_dictionary_2
                    and tablerowid_1 in table_dictionary_2[tableid]
                ):
                    row_occurrences = table_dictionary_2[tableid][
                        tablerowid_1
                    ]  # list of [[colid, token]]
                    for row_occurrence in row_occurrences:
                        ...
                        for relevant_row in relevant_rows:
                            if token_2 == relevant_row[1]:
                                if (
                                    "{}_{}_{}".format(tableid, colid_1, colid_2)
                                    in joinability_dictionary
                                ):
                                    joinability_dictionary[
                                        "{}_{}_{}".format(tableid, colid_1, colid_2)
                                    ] += 1
        ...
        top_joinables = sorted(
            joinability_dictionary,
            key=lambda k: joinability_dictionary[k],
            reverse=True,
        )[: self.top_k]
        ll = []
        ...
        info_to_store = "fetching_time_1 = {}, fetching_time_2 = {}, evaluation_time = {}, joinability_average = {} , ".format(
            fetching_time_1, fetching_time_2, evaluation_time, joinability_average
        )
    ...
    def run_system_bf(
        self, hash_size: int = 128, run_ics: bool = False, active_pruning: bool = True
    ) -> int:
        ...
        for tableid in tqdm(
            ...
            for hit in sorted(hitting_posting_list_concatenated):
                ...
                for input_row in relevant_input_rows:
                    if (int(input_row[super_key_index], 2) | int(superkey, 2)) == int(
                        superkey, 2
                    ):
                        candidate_external_row_ids += [rowid]
                        set_of_rowids.add(rowid)
...
if __name__ == "__main__":
    ...
    MATETableExtraction(
        "movie",
        os.path.join(os.path.dirname(__file__), "..", "datasets", "movie.csv"),
        ["director_name", "movie_title"],
        top_k,
        "main_tokenized",
        one_bits,
        f"MATE_datasets_k_bits_ones_{top_k}_{bits}_{one_bits}",
    ).MATE(bits)
