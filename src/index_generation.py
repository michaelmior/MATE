...
def XASH(token: str, hash_size: int = 128) -> int:
    ...
    selected_chars = [
        y[0]
        for y in sorted(cnt_dict.items(), key=lambda x: (x[1], x[0]), reverse=False)[
            :number_of_ones
        ]
    ]
...
def generate_index(
    ...
    for row in cur.fetchall():
        ...
        for rowid in np.arange(rowid_max):
            cur.execute(
                f"SELECT tokenized FROM {main_table} WHERE tableid = {table_id} AND rowid = {rowid};"
            )
            ...
            cur.execute(
                f"UPDATE main_tokenized SET {super_key_column} = {super_key} "
